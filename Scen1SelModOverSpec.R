...
for (i in 1:nsims) {
	...
	svysamp$prob.subsel <- expit(beta0+ beta1*svysamp$z1 + beta3*I(svysamp$z1^2) )
...
write.table(effects, file="EffectsS1MMOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(bootvar, file="VarS1MMOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(coverage.ate, file="CoverageATES1MMOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")