...
effects <- bootvar <- coverage.ate <- matrix(NA, ncol=2, nrow=nsims)
colnames(effects) <- colnames(bootvar) <- colnames(coverage.ate) <- c( "TMLE3", "TMLE3over")
...
## TRUE
###IPW
effects[i, "IPW"] <-ipsw(svysamp)
res.ipw<-unlist(mclapply(1:trials, boot.ipsw, mc.cores=6))
...
write.table(effects, file="EffectsS1MMTxOver2.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(bootvar, file="VarS1MMTxOver2.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(coverage.ate, file="CoverageATES1MMTxOver2.csv", row.names=FALSE, col.names=TRUE, sep=",")