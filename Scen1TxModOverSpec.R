...
source("TMLEfunctionsMScen1TxAnalyses.R")
source("TMLEfunctionsMestScen1TxAnalyses.R")
...
#P(tx ~ 1/3)
prob.trt<-expit(-2.5 + (log(2.35)*z2) )
...
###IPW
effects[i, "IPW"] <-ipw.less1(svysamp)
res.ipw<-unlist(mclapply(1:trials, boot.ipw.less1, mc.cores=6))
bootvar[i, "IPW"] <-var(res.ipw)
cov<-quantile(res.ipw, probs=c(.025, .975))
coverage.ate[i,"IPW"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

###TMLE	
effects[i, "TMLE2"] <-joffe.less1(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffe.less1, mc.cores=6))
bootvar[i, "TMLE2"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3"] <-tmle3.less1(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3.less1, mc.cores=6))
...
write.table(effects, file="EffectsS1MMTxOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(bootvar, file="VarS1MMTxOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(coverage.ate, file="CoverageATES1MMTxOver1.csv", row.names=FALSE, col.names=TRUE, sep=",")