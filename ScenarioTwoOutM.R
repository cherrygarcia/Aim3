...
colnames(effects) <- colnames(bootvar) <- colnames(coverage.ate) <- c("TMLE2O1", "TMLE3O1","TMLE2O2", "TMLE3O2", "TMLE2O3", "TMLE3O3")
...
effects[i, "TMLE2O1"] <-joffeO1(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffeO1, mc.cores=6))
bootvar[i, "TMLE2O1"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2O1"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3O1"] <-tmle3O1(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3O1, mc.cores=6))
bootvar[i, "TMLE3O1"] <-var(res.tmle3)
cov<-quantile(res.tmle3, probs=c(.025, .975))
coverage.ate[i,"TMLE3O1"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE2O2"] <-joffeO2(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffeO2, mc.cores=6))
bootvar[i, "TMLE2O2"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2O2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3O2"] <-tmle3O2(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3O2, mc.cores=6))
bootvar[i, "TMLE3O2"] <-var(res.tmle3)
cov<-quantile(res.tmle3, probs=c(.025, .975))
coverage.ate[i,"TMLE3O2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE2O3"] <-joffeO3(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffeO3, mc.cores=6))
bootvar[i, "TMLE2O3"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2O3"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3O3"] <-tmle3O3(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3O3, mc.cores=6))
bootvar[i, "TMLE3O3"] <-var(res.tmle3)
cov<-quantile(res.tmle3, probs=c(.025, .975))
coverage.ate[i,"TMLE3O3"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

}

write.table(effects, file="EffectsS2MMOut.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(bootvar, file="VarS2MMOut.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(coverage.ate, file="CoverageATES2MMOut.csv", row.names=FALSE, col.names=TRUE, sep=",")