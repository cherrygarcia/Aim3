...
source("TMLEfunctionsM.R")
source("TMLEfunctionsMest.R")
...
colnames(effects) <- colnames(bootvar) <- colnames(coverage.ate) <- c("IPWT1",  "TMLE2T1", "TMLE3T1", "IPWT2", "TMLE2T2", "TMLE3T2")
...
###IPW	
effects[i, "IPWT1"] <-ipwT1(svysamp)
res.ipw<-unlist(mclapply(1:trials, boot.ipwT1, mc.cores=6))
bootvar[i, "IPWT1"] <-var(res.ipw)
cov<-quantile(res.ipw, probs=c(.025, .975))
coverage.ate[i,"IPWT1"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "IPWT2"] <-ipwT2(svysamp)
res.ipw<-unlist(mclapply(1:trials, boot.ipwT2, mc.cores=6))
bootvar[i, "IPWT2"] <-var(res.ipw)
cov<-quantile(res.ipw, probs=c(.025, .975))
coverage.ate[i,"IPWT2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

###TMLE

effects[i, "TMLE2T1"] <-joffeT1(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffeT1, mc.cores=6))
bootvar[i, "TMLE2T1"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2T1"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3T1"] <-tmle3T1(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3T1, mc.cores=6))
bootvar[i, "TMLE3T1"] <-var(res.tmle3)
cov<-quantile(res.tmle3, probs=c(.025, .975))
coverage.ate[i,"TMLE3T1"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE2T2"] <-joffeT2(svysamp)
res.joffe<-unlist(mclapply(1:trials, boot.joffeT2, mc.cores=6))
bootvar[i, "TMLE2T2"] <-var(res.joffe)
cov<-quantile(res.joffe, probs=c(.025, .975))
coverage.ate[i,"TMLE2T2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

effects[i, "TMLE3T2"] <-tmle3T2(svysamp)
res.tmle3<-unlist(mclapply(1:trials, boot.tmle3T2, mc.cores=6))
bootvar[i, "TMLE3T2"] <-var(res.tmle3)
cov<-quantile(res.tmle3, probs=c(.025, .975))
coverage.ate[i,"TMLE3T2"] <-  ifelse(cov[1] < ate & cov[2] > ate, 1, 0)

}

write.table(effects, file="EffectsS2MMTx.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(bootvar, file="VarS2MMTx.csv", row.names=FALSE, col.names=TRUE, sep=",")
write.table(coverage.ate, file="CoverageATES2MMTx.csv", row.names=FALSE, col.names=TRUE, sep=",")