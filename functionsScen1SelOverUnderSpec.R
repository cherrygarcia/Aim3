...
joffe.under<- function(svysamp){
	...
	#combine weights
	sampled.data$subsvytrtwt<-ifelse(sampled.data$insubsample==1, sampled.data$t.wt*sampled.data$ipsvyselwt, 0)
...
joffe.less1<- function(svysamp){
	...
	sampled.data$pscore.subsamp1[sampled.data$t==1]<-predict(glm(insubsample ~ poly(z1,2), data=sampled.data[sampled.data$t==1,], family="binomial"), type="response")
	sampled.data$subsamp.wt[sampled.data$t==1]<-1/sampled.data$pscore.subsamp1[sampled.data$t==1]
	
	sampled.data$pscore.subsamp1[sampled.data$t==0]<-predict(glm(insubsample ~ poly(z1,2) , data=sampled.data[sampled.data$t==0,], family="binomial"), type="response")
...
tmle3.under<-function(svysamp){
	sampled.data<-svysamp
	sampled.data$pscore.svy <- predict(glm(t ~ poly(z2,2), data=sampled.data, family="binomial"), type="response")
	
	modelc<-glm(insubsample ~ 1 , data=sampled.data, family="binomial")
...
tmle3.less1<-function(svysamp){
	...
	modelc<-glm(insubsample ~ poly(z1,2) , data=sampled.data, family="binomial")
